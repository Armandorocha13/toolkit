...
async function exportFullDatasetSync() {
    let allRecords = [];
    let from = 0;
    const step = 800;
    let hasMore = true;

    console.log("📥 Iniciando exportação MASSIVA com Saldo Sincronizado...");
    ...
    const { data: precos } = await supabase.rpc('execute_sql', { query: "SELECT codigo, valor FROM public.apoio_materiais_familia" });
    const precosMap = {};
    if (precos) {
        const parsedPrecos = JSON.parse(precos.result);
        parsedPrecos.forEach(p => precosMap[p.codigo] = p.valor);
    }

    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Relatório Sincronizado');
    ...
    const finalizedRows = allRecords.map(r => ({
        ...r,
        unitario: precosMap[r.codigo_item] || 0
    }));

    worksheet.addRows(finalizedRows);
    worksheet.getRow(1).font = { bold: true };

    const filePath = 'c:\\Users\\user\\Desktop\\ARQUVOS\\PPROJETOS PROGRAMAÇÃO\\Listagem Toolkit\\dados\\view_consolidada_FINAL_Sincronizada.xlsx';
    await workbook.xlsx.writeFile(filePath);
    console.log(`✅ EXPORTAÇÃO (VERSÃO SINCRONIZADA) CONCLUÍDA!`);
    console.log(`📊 TOTAL DE LINHAS: ${finalizedRows.length}`);
}

exportFullDatasetSync();
